# 每换一个后台系统就重写脚本？我用 Playwright + YAML Flow 做了一套通用自动化框架
...
配图建议：`articles-publish/diagrams/01-framework-overview.html`
...
| 流程来源 | 录制、拖拽配置、流程设计器 | SOP、图文、trace、YAML Flow、Recording Pack |
...
这就是 V1 已经先落地的 `Recorder / Capture Layer` 方向。

早期 SAP Adapter 主要来自 SOP、图文、人工操作复盘和 Playwright trace；V1 之后，Recording Pack 已经先把材料目录、元数据、草稿生成和 review checklist 标准化。完整 Recorder 仍然可以继续增强，但不再是完全空白。

## 当前短板：Recording Pack 已有 V1，完整 Recorder 仍要增强

这也是当前项目最需要诚实说明的一点。

现在的 SAP / SRM Adapter 已经有真实工程证据：主线 Flow、Page Object、Action / Adapter capability、单元测试、Flow scanner，以及多轮企业后台踩坑修复。但最早一批自动化资产主要来自：
...
这条路径能跑通第一个 SAP 样例，但不是最理想的“自动化资产生产方式”。

所以 V1 先把采集层落成了可审查的 Recording Pack：
...
更完整的链路仍然应该继续往自动 Recorder 和编译器增强：
...
所以这套框架当前已经完成的不只是执行内核，还包括 Recording Pack V1。下一步要补的是更自动化的采集、证据提取和编译增强。
...
| 第一个 Adapter | SAP ECC / SAP WebGUI |
| 第二个 Adapter | SRM production skeleton，query / create / confirm / generate / upload 已完成第一轮收口 |
| 单元测试 | action / adapter / capability / flow-loader 等关键路径已有定向回归 |
| Skill | `sap-ui-auto` v3.10，包含 SRM 能力边界和真实操作限制 |
| 当前阶段 | V1~V3 主线闭环完成，进入发布、归档治理和真实验证准备 |
...
| Flow / Adapter capability scanner | 已实现 | 治理工具 |
| SRM 第二 Adapter production skeleton | V3 第一轮完成 | 领域适配层 |
...
## Core vs Adapter：一句话判断

```text
Core = 所有企业系统都要的能力（Flow、Runtime、Report、Trace、Dry-run、AI Diagnose）
Adapter = 某个系统特有的能力（iframe、TCode、readonly、审批流、供应商门户）
```

具体每层怎么设计，后续篇目会逐一展开。这里只需要记住：通用问题进 Core，系统差异进 Adapter，业务流程进 Flow。
...
推荐先看系列引子篇《从"能跑一个 SAP 事务"到"换系统不重写"》了解完整项目演进，再按兴趣跳到具体设计篇目。
