...
export const SAP_SRM_CAPABILITIES: AdapterCapability[] = [
  ...
  },
  {
    ...
    evidence: ['uploaded attachment is visible in SRM', 'business object id is captured', 'file path and content review approval are recorded'],
    notes: 'Approval-gated production skeleton. Upload preflight requires an existing allowed file and sensitive_content_reviewed=true. Real SRM environment validation is still required before operational use.',
  },
  {
    name: 'uploadPOScanLegacyOperation',
    action: 'srm_operation',
    method: 'uploadPOScan',
    risk: 'reversible_change',
    status: 'blocked',
    requiresHumanApproval: true,
    evidence: ['legacy operation is rejected before upload'],
    notes: 'Retired legacy srm_operation wrapper. New Flows must use srm_upload_po_scan with explicit file_path and sensitive content review.',
  },
  {
